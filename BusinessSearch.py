...
def get_businesses():
    ...
    final = {business["name"]: business["rating"]
             for business in businesses if business["rating"] >= rest_rate}
    final = sorted(final.items(), key=lambda k: k[0])
...
location_frame.place(relx=0.5, rely=0.05, relwidth=0.8,
                     relheight=0.15, anchor="n")

location_label = Label(location_frame, text="Location Indicator",
                       bg="#6c4cfc", fg="white", font=("Century", 20))
...
business_frame.place(relx=0.5, rely=0.2, relwidth=0.8,
                     relheight=0.15, anchor="n")

business_label = Label(business_frame, text="Type of Business",
                       bg="#6c4cfc", fg="white", font=("Century", 20))
...
ratings_frame.place(relx=0.5, rely=0.35, relwidth=0.8,
                    relheight=0.15, anchor="n")

ratings_label = Label(ratings_frame, text="Stars Out of 5.0",
                      bg="#6c4cfc", fg="white", font=("Century", 20))
...
find_button = Button(find_frame, text="GO!", bg="white",
                     command=get_businesses, font=("Century", 20))
...
results_frame.place(relx=0.5, rely=0.7, relwidth=0.8,
                    relheight=0.2, anchor="n")

scrlbr = Scrollbar(results_frame, orient="vertical")

results_listbox = Listbox(
    results_frame, yscrollcommand=scrlbr.set, font=("Century", 10))
